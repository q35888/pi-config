---
name: attack-chain
description: 多阶段攻击链编排总指挥。当任务需要"从 A 打到 B"的完整渗透链路时使用，负责编排各阶段、协调子 skill、规划攻击路径。任何需要跨阶段组合的渗透场景（横向移动、权限提升、内网穿透）都从此处开始。触发：攻击链、kill chain、多阶段渗透、横向移动、攻击路径规划、红队编排。
---
