---
name: zzy-reverse-master
description: zzy-reverse 逆向工程技能路由主控制器。当不确定路由到哪个具体逆向子 skill，或任务跨多个子 skill 需要总编排时使用。读取 routing.md 按"目标类型+用户意图+工具链"三轴分发到具体子 skill。作为内部路由中枢，不直接面向终端任务。
---
